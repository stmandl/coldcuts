...
template <typename T>
T myreduce(vector<T> v, function<T(T,T)>f, T init) {    
    if (v.size()==0) 
    return init;
    T res = init;
...
int main() {
  ...
  cout << a << endl;
  cout << b << endl;
  cout << myreduce(b, std::function([](int a, int b)->int{return a+b;}), 0) << endl;
  cout << myreduce(b, std::function(myplus), 0) << endl;
  cout << myreduce(b, std::function([](int a, int b)->int{return a*b;}), 1) << endl;
  return 0;
}